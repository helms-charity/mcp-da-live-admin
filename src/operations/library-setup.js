...
async function processBlockDocumentation(octokit, org, repo, blockName, blocksPath, baseFolder, branch, description, sourceDocuments, docExists, preview) {
  if (docExists) {
    return {
      step: 4,
...
async function addBlock(args) {
  const { org, repo, blockName, baseFolder, branch, blocksPath, description, sourceDocuments, preview } = args;
  ...
  try {
    ...
    const docStep = await processBlockDocumentation(
      octokit, org, repo, blockName, blocksPath, baseFolder, branch, description, sources, docExists, preview
    );
...
async function processBlock(octokit, org, repo, blockName, blocksPath, baseFolder, branch, sourceDocuments, autoGenerateDocs, preserveExisting, preview) {
  const blockResult = {
    name: blockName,
    action: 'unknown',
    docPath: null,
...
async function setup(args) {
  const { org, repo, baseFolder, branch, blocksPath, filter, sourceDocuments, autoGenerateDocs, preserveExisting, preview } = args;
  const octokit = createGitHubClient();
  const sources = normalizeSourceDocuments(sourceDocuments);
    ...
    for (const dir of directories) {
      try {
        const blockResult = await processBlock(
          octokit, org, repo, dir.name, blocksPath, baseFolder, branch, sources,
